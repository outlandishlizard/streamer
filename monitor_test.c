...
void *getter (void __attribute__((unused)) *a) {
  while (1) {
    char *result;
    monitor_run_fn(m_str, monitor_get, &result);
    monitor_run_fn(m_out, monitor_print, result);
...
int main (void) {
  m_str = monitor_create();
  m_out = monitor_create();
  full = monitor_cond_create(m_str);
  empty = monitor_cond_create(m_str);
